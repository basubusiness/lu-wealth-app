...
def run_analysis():
    ...
    if res.success:
        ...
        # This loop handles both V1 (step_up=0) and V2 (step_up>0)
...
if st.session_state.results:
    ...
    with t1:
        ...
        st.table(pd.DataFrame({"Asset": res["names"], "Target %": res["w"], "Lump Sum": res["w"]*current_val, "Monthly": res["w"]*monthly_add})[lambda x: x["Target %"] > 0.005].style.format({"Target %":"{:.1%}", "Lump Sum":"€{:,.0f}", "Monthly":"€{:,.0f}"}))
    ...
    with c_n1:
        ...
        if step_up > 0:
            st.caption(f"💡 Year {horizon} monthly contribution: €{monthly_add * (1+step_up)**(horizon-1):,.0f}")
        if res["tip"]: 
