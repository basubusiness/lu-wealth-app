...
# --- 2. DATA & CORRELATION ---
asset_list = ["World Equity", "All-World", "World Small Cap", "US (S&P 500)", "Europe 600", "Emerging Mkts", "Euro Gov Bonds", "Corp Bonds", "Cash/MM", "Global REITs", "Gold", "Crypto"]
default_market_data = {
...
c1, c2 = st.columns(2)
with c1:
    target_ret = st.number_input("Target Return %", 1.0, 25.0, 8.5, help="Target for the optimizer to hit with the lowest risk.") / 100
    current_val = st.number_input("Initial Capital (€)", value=100000)
    step_up = st.slider("Annual Contribution Increase %", 0, 15, 3, help="In V2, your monthly savings grow by this % every year.") / 100 if mode == "V2 (Annual Step-Up)" else 0.0
with c2:
    ...
    adj_inflation = st.checkbox("Show Chart in Today's Euros (Inflation Adjusted)", value=False)

conf_level = st.select_slider("Stress Confidence", [0.90, 0.95, 0.99], 0.95, help="Used for the Stress Floor calculation.")
...
def run_analysis():
    ...
    if res.success:
        ...
        gns = [0] + [wth[i]-wth[i-1]-((monthly_add*12)*((1+step_up)**(max(0,i-1)))/((1+inflation)**i if adj_inflation else 1)) for i in range(1, len(y))]
        tip = next((t for t, g in enumerate(gns) if g > ((monthly_add*12)*((1+step_up)**(max(0,t-1)))/((1+inflation)**t if adj_inflation else 1))), None)
...
if st.session_state.results:
    ...
    with t1:
        ...
        st.metric("Final Wealth", f"€{tw:,.0f}", f"+{(tw/tp-1)*100:.1f}% Profit", help="Estimated total value at the end of your horizon.")
        final_monthly = monthly_add * ((1 + step_up)**(horizon-1))
        mix_df = pd.DataFrame({"Asset": res["names"], "Target %": res["w"], "Lump Sum": res["w"]*current_val, "Year 1 Monthly": res["w"]*monthly_add, f"Year {horizon} Monthly": res["w"]*final_monthly})
        st.table(mix_df[mix_df["Target %"] > 0.005].style.format({"Target %":"{:.1%}", "Lump Sum":"€{:,.0f}", "Year 1 Monthly":"€{:,.0f}", f"Year {horizon} Monthly":"€{:,.0f}"}))
    ...
    with t5:
        ...
        fig_w = go.Figure(go.Waterfall(name="Risk", orientation="v", measure=["relative"]*len(risk_df[risk_df["Risk Contribution %"]>0.001]), x=risk_df[risk_df["Risk Contribution %"]>0.001]["Asset"], y=risk_df[risk_df["Risk Contribution %"]>0.001]["Risk Contribution %"]*100))
        st.plotly_chart(fig_w, use_container_width=True)
        st.info("💡 **Risk Waterfall:** This shows which assets are contributing most to your portfolio volatility. High-weight assets with low risk contribution are your best diversifiers.")

    # --- 8. RESTORED NARRATIVE & TOOLTIPS ---
    st.divider()
    ...
    with c_n1:
        ...
        st.metric("Monthly Payout (Real)", f"€{max(0, payout):,.0f}", help="The 'Safe Withdrawal' amount in today's purchasing power while keeping principal intact.")
        if res["tip"]: 
            st.write(f"✅ **Tipping Point: Year {res['tip']}**")
            st.caption("The exact year your portfolio's annual market growth exceeds your own annual savings. Your wealth is now self-sustaining.")
    with c_n2:
        st.subheader("🛡️ Safety & Tax")
        st.metric("Stress Floor", f"€{worst:,.0f}", help=f"Statistically, there is a {conf_level*100:.0f}% chance your wealth remains above this level even in a major downturn.")
        st.write(f"💰 **LU Tax Advantage: €{(res['wth'][-1]-res['inv'][-1]) * 0.40:,.0f}**")
        st.caption("Estimated savings assuming a 0% tax rate on long-term capital gains in Luxembourg vs. a 40% regime elsewhere.")